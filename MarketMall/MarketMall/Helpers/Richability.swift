...
public class Reachabilty {
    ...
    class func HasConnection() -> Bool {
        ...
        let defaultRoutReachbility = withUnsafePointer(to: &zeroAddress, {$0.withMemoryRebound(to: sockaddr.self, capacity: 1, {
            zeroSockAddress in
        ...
        if SCNetworkReachabilityGetFlags(defaultRoutReachbility!, &flags) == false {
            return false
        }
